...
void ftp_reply(session_t *sess, int stat, const char * text);
void ftp_lreply(session_t *sess, int stat, const char * text);
...
void handle_child(session_t *sess)
{
	...
	while(1)
	{
		...
		//jie xi FTP ming lin yu can shu 
		str_split(sess->cmdline, sess->cmd, sess->arg, ' ');
		printf("cmd=[%s], arg=[%s]\n", sess->cmd, sess->arg);		

		//jiang ming lin zhuan huan cheng da xie
		str_upper(sess->cmd);
...
int get_transfer_fd(session_t *sess)
{
	...
	if(!port_active(sess) && !pasv_active(sess))
	{
		...
		ftp_reply(sess, FTP_BADSENDCONN, "Using PORT or PASV first.");
		return 0;
	}
	...
	if(port_active(sess))
	{
		...
		bind(20);
		connect;*/
		//tcp_client(20);由于客户进程没有权限绑定20端口 所以会失败，暂时不绑定
...
static void do_user(session_t *sess)
{
	...
	sess->uid = pw->pw_uid;
	ftp_reply(sess, FTP_GIVEWORD, "please specify the password.");
}

static void do_pass(session_t *sess)
{
	//writen(sess->ctrl_fd, "230 Longin successful.\r\n", strlen("230 Longin successful.\r\n"));
	//pass 123456
	...
	// jiang ming wen jin xing jia mi
...
static void do_port(session_t *sess)
{
	...
	if(sess->port_addr == NULL)
		ERR_EXIT("NO SPACE MALLOC DO_PORT");
...
static void do_type(session_t *sess)
{
	...
	else if(strcmp(sess->arg, "I") == 0)
	{
		...
		ftp_reply(sess, FTP_BADCMD, "Unrecognised TYPE commmand.");
...
static void do_feat(session_t *sess)
{
	ftp_lreply(sess, FTP_FEATOK, "Features:");
